<h1 align="center">Hi 👋, I'm Gugan</h1>

<h3 align="center">
🚀 AI Engineer | Agentic AI | LLMs | Building Intelligent Systems
</h3>

<p align="center">
<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=22&duration=3000&color=00F7FF&center=true&vCenter=true&width=600&lines=AI+Engineer;LLMs+%7C+RAG+%7C+Agentic+AI;Building+Real-World+AI+Systems;Always+Learning+%26+Building" />
</p>

<p align="center">
<img src="https://komarev.com/ghpvc/?username=gugan-web&label=Profile%20views&color=0e75b6&style=flat" />
</p>

---

## 🧠 About Me
- 🎓 Pre-final year ECE student focused on **AI + Embedded Systems**
- 🤖 Building with **LLMs, RAG, AI Agents, Agentic AI**
- ⚡ Strong in **DSA, System Design & Problem Solving**
- 🔍 Interested in **AI Research + Real-world Systems**
- 📫 Reach me: **t.gugan2005@gmail.com**

---

## 🌐 Connect With Me
<p align="center">
<a href="https://linkedin.com/in/gugan69"><img src="https://skillicons.dev/icons?i=linkedin" height="40"/></a>
<a href="https://www.leetcode.com/6t0sk0zyzk"><img src="https://img.icons8.com/external-tal-revivo-color-tal-revivo/24/external-level-up-your-coding-skills-and-quickly-land-a-job-logo-color-tal-revivo.png" height="40"/></a>
<a href="https://www.hackerrank.com/czepxfod"><img src="https://skillicons.dev/icons?i=hackerrank" height="40"/></a>
<a href="https://auth.geeksforgeeks.org/user/tguganvguh"><img src="https://cdn.worldvectorlogo.com/logos/geeksforgeeks-1.svg" height="40"/></a>
</p>

---

## ⚙️ Tech Stack
<p align="center">
<img src="https://skillicons.dev/icons?i=python,cpp,go,html,css,linux,docker,kubernetes,aws,gcp,firebase,mysql,git,jenkins,bash" />
</p>

---

## 🤖 AI Stack
<p align="center">
<img src="https://skillicons.dev/icons?i=pytorch,tensorflow" />
<img src="https://cdn.worldvectorlogo.com/logos/opencv-icon.svg" height="48"/>
<img src="https://cdn.worldvectorlogo.com/logos/pandas.svg" height="48"/>
<img src="https://upload.wikimedia.org/wikipedia/commons/0/05/Scikit_learn_logo_small.svg" height="48"/>
</p>

---

# 🚀 Featured Projects 

### 🧠 AI Projects

#### 🔹 AI Agent System
- Built autonomous AI agents using LLMs
- Supports planning, reasoning & tool usage
- Tech: Python, LLM APIs, LangChain

#### 🔹 RAG Pipeline System
- Retrieval-Augmented Generation system
- Integrated vector DB + semantic search
- Improved factual accuracy of LLM outputs

#### 🔹 Knowledge-Augmented AI (KAG)
- Combined knowledge graphs with LLMs
- Enhanced reasoning + structured intelligence

---

### 💻 Software / Systems Projects

#### 🔹 Home Automation System
- IoT-based smart control system
- Arduino + sensors + real-time control

#### 🔹 Bank Data Management System
- Secure backend system for transaction handling
- Focus on data integrity & efficiency

#### 🔹 ML Classifier (Cat vs Dog)
- Image classification using CNN
- Built using PyTorch / TensorFlow

---

> 💡 Tip: Replace these with your actual GitHub repo links (VERY IMPORTANT)

---

## 📊 GitHub Analytics

<p align="center">
<img src="https://github-readme-stats.vercel.app/api?username=gugan-web&show_icons=true&theme=tokyonight" />
<img src="https://github-readme-streak-stats.herokuapp.com/?user=gugan-web&theme=tokyonight" />
</p>

<p align="center">
<img src="https://github-readme-stats.vercel.app/api/top-langs?username=gugan-web&layout=compact&theme=tokyonight" />
</p>

---


## 🔥 What I'm Currently Doing
- 🚀 Building **Agentic AI systems**
- 🧠 Learning **advanced LLM architectures**
- 📚 Exploring **RAG, KAG & AI pipelines**
- ⚡ Practicing **DSA & system design**

---

## 💡 Vision
> "I aim to build intelligent systems that can think, reason, and act autonomously."

---
